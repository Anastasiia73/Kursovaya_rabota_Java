...
public class User implements UserDetails {
    ...
    public int getPCount(int pizzaNumber){
        for (Cart item : list){
            if (item.getPizzaNumber() == pizzaNumber){
                return item.getpizzaCount();
            }
        }
        return 0;
    }

    public Cart getP(int bouquerNumber){
        for (Cart item: list){
            if (item.getPizzaNumber() == bouquerNumber)
                return item;
        }
        return null;
    }
